...
export async function GET(request: Request) {
  try {
    ...
    const builder = query ? `?q=${encodeURIComponent(query)}` : ''
    const data = await fetchFromDjango(`/products/search/${builder}`, {}, request.headers)
    return NextResponse.json(data)
  } catch (error) {
    return NextResponse.json({ error: error instanceof Error ? error.message : 'Failed to search products' }, { status: 502 })
  }
}
