...
export async function GET(request: Request, { params }: { params: { product_id: string } }) {
  try {
    const data = await fetchFromDjango(`/products/${params.product_id}/`, {}, request.headers)
    return NextResponse.json(data)
  } catch (error) {
    return NextResponse.json({ error: error instanceof Error ? error.message : 'Failed to fetch product detail' }, { status: 502 })
  }
}
...
export async function PUT(request: Request, { params }: { params: { product_id: string } }) {
  try {
    ...
    const data = await fetchFromDjango(`/products/${params.product_id}/`, {
      method: 'PUT',
    ...
    return NextResponse.json({ error: error instanceof Error ? error.message : 'Failed to update product' }, { status: 502 })
  }
}

export async function PATCH(request: Request, { params }: { params: { product_id: string } }) {
  try {
    const body = await request.json()
    const data = await fetchFromDjango(`/products/${params.product_id}/`, {
      method: 'PATCH',
...
export async function DELETE(request: Request, { params }: { params: { product_id: string } }) {
  try {
    ...
    return NextResponse.json({ error: error instanceof Error ? error.message : 'Failed to delete product' }, { status: 502 })
  }
}
