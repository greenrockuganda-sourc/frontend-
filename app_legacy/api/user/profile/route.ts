...
export async function GET(request: Request) {
  try {
    const data = await fetchFromDjango('/user/profile/', {}, request.headers)
...
export async function PUT(request: Request) {
  try {
    ...
    const data = await fetchFromDjango('/user/profile/', {
      method: 'PUT',
    ...
    return NextResponse.json({ error: error instanceof Error ? error.message : 'Failed to update profile' }, { status: 502 })
  }
}
