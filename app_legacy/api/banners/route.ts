...
export async function GET(request: Request) {
  try {
    const data = await fetchFromDjango('/banners/', {}, request.headers)
    return NextResponse.json(data)
  } catch (error) {
    return NextResponse.json({ error: error instanceof Error ? error.message : 'Failed to fetch banners' }, { status: 502 })
  }
}
...
export async function POST(request: Request) {
  try {
    ...
    const data = await fetchFromDjango('/banners/', {
      method: 'POST',
    ...
    return NextResponse.json({ error: error instanceof Error ? error.message : 'Failed to create banner' }, { status: 502 })
  }
}
