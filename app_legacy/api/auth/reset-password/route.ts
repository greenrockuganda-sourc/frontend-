...
export async function POST(request: Request) {
  try {
    ...
    const data = await fetchFromDjango('/auth/reset-password/', {
      method: 'POST',
    ...
    return NextResponse.json({ error: error instanceof Error ? error.message : 'Failed to reset password' }, { status: 502 })
  }
}
