...
export async function GET(request: Request) {
  try {
    const data = await fetchFromDjango('/recipes/', {}, request.headers)
    return NextResponse.json(data)
  } catch (error) {
    return NextResponse.json({ error: error instanceof Error ? error.message : 'Failed to fetch recipes' }, { status: 502 })
  }
}
...
export async function POST(request: Request) {
  try {
    ...
    const data = await fetchFromDjango('/recipes/', {
      method: 'POST',
    ...
    return NextResponse.json({ error: error instanceof Error ? error.message : 'Failed to create recipe' }, { status: 502 })
  }
}
