...
export async function GET(request: Request, { params }: { params: { category_id: string } }) {
  try {
    const data = await fetchFromDjango(`/categories/${params.category_id}/`, {}, request.headers)
    return NextResponse.json(data)
  } catch (error) {
    return NextResponse.json({ error: error instanceof Error ? error.message : 'Failed to fetch category detail' }, { status: 502 })
  }
}
...
export async function PUT(request: Request, { params }: { params: { category_id: string } }) {
  try {
    ...
    const data = await fetchFromDjango(`/categories/${params.category_id}/`, {
      method: 'PUT',
    ...
    return NextResponse.json({ error: error instanceof Error ? error.message : 'Failed to update category' }, { status: 502 })
  }
}

export async function PATCH(request: Request, { params }: { params: { category_id: string } }) {
  try {
    const body = await request.json()
    const data = await fetchFromDjango(`/categories/${params.category_id}/`, {
      method: 'PATCH',
...
export async function DELETE(request: Request, { params }: { params: { category_id: string } }) {
  try {
    ...
    return NextResponse.json({ error: error instanceof Error ? error.message : 'Failed to delete category' }, { status: 502 })
  }
}
