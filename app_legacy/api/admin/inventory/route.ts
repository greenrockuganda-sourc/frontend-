...
export async function GET(request: Request) {
  try {
    const data = await fetchFromDjango('/admin/inventory/', {}, request.headers)
    return NextResponse.json(data)
  } catch (error) {
    return NextResponse.json({ error: error instanceof Error ? error.message : 'Failed to fetch inventory' }, { status: 502 })
  }
}
...
export async function POST(request: Request) {
  try {
    ...
    const data = await fetchFromDjango('/admin/inventory/', {
      method: 'POST',
    ...
    return NextResponse.json({ error: error instanceof Error ? error.message : 'Failed to update inventory' }, { status: 502 })
  }
}
