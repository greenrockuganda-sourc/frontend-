...
export async function PUT(request: Request, { params }: { params: { product_id: string } }) {
  try {
    ...
    const data = await fetchFromDjango(`/admin/inventory/${params.product_id}/`, {
      method: 'PUT',
    ...
    return NextResponse.json({ error: error instanceof Error ? error.message : 'Failed to update inventory item' }, { status: 502 })
  }
}
