...
export async function GET(request: Request) {
  try {
    const data = await fetchFromDjango('/admin/dashboard/', {}, request.headers)
    return NextResponse.json(data)
  } catch (error) {
    return NextResponse.json({ error: error instanceof Error ? error.message : 'Failed to fetch dashboard stats' }, { status: 502 })
  }
}
