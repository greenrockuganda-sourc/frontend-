...
export async function GET(request: Request, { params }: { params: { customer_id: string } }) {
  try {
    const data = await fetchFromDjango(`/admin/customers/${params.customer_id}/`, {}, request.headers)
    return NextResponse.json(data)
  } catch (error) {
    return NextResponse.json({ error: error instanceof Error ? error.message : 'Failed to fetch customer detail' }, { status: 502 })
  }
}

export async function PUT(request: Request, { params }: { params: { customer_id: string } }) {
  try {
    const body = await request.json()
    const data = await fetchFromDjango(`/admin/customers/${params.customer_id}/`, {
      method: 'PUT',
    ...
    return NextResponse.json({ error: error instanceof Error ? error.message : 'Failed to update customer' }, { status: 502 })
  }
}
