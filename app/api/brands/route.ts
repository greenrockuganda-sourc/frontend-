...
export async function GET(request: Request) {
  try {
    const data = await fetchFromDjango('/brands/', {}, request.headers)
    return NextResponse.json(data)
  } catch (err) {
    return NextResponse.json({ error: 'Failed to fetch brands from backend' }, { status: 502 })
  }
}
...
export async function POST(request: Request) {
  try {
    ...
    if (!body || !body.brand_name || String(body.brand_name).trim() === '') {
      return NextResponse.json({ error: 'brand_name is required' }, { status: 400 })
    }

    const created = await fetchFromDjango('/brands/', { method: 'POST', body: JSON.stringify(body) }, request.headers)
    return NextResponse.json(created, { status: 201 })
  } catch (err) {
    return NextResponse.json({ error: 'Failed to create brand on backend' }, { status: 502 })
  }
}
