...
import { ArrowUpRight, Clock3, Filter, Pencil, Plus, Search, Sparkles, Tag, Trash2 } from 'lucide-react'
...
export default function Brands({ onNavigate }: Props) {
  ...
  const [pendingDeleteBrand, setPendingDeleteBrand] = useState<any | null>(null)
  ...
  const handleDeleteBrand = async () => {
    ...
    try {
      await brandsApi.delete(String(pendingDeleteBrand.id))
      setItems((prev) => prev.filter((item) => String(item.id) !== String(pendingDeleteBrand.id)))
      notifySuccess('Brand deleted successfully')
    } catch (error) {
      notifyError(error instanceof Error ? error.message : 'Unable to delete brand.')
    } finally {
  ...
  return (
    <main className="min-h-screen bg-white text-slate-950">
            ...
            <div className="overflow-x-auto">
              <table className="min-w-full border-separate border-spacing-0 text-left">
                <thead className="bg-slate-50">
                  <tr>
                    <th className="px-6 py-3 text-xs font-semibold uppercase tracking-[0.18em] text-slate-500">Brand</th>
                    <th className="px-6 py-3 text-xs font-semibold uppercase tracking-[0.18em] text-slate-500">Updated</th>
                    <th className="hidden px-6 py-3 text-xs font-semibold uppercase tracking-[0.18em] text-slate-500 md:table-cell">Created</th>
                    <th className="py-3 pr-6 pl-2 text-right text-xs font-semibold uppercase tracking-[0.18em] text-slate-500">Actions</th>
                  </tr>
                </thead>
                <tbody>
                  {filteredItems.map((brand) => (
                    <tr key={brand.id} className="border-t border-slate-200/80 bg-white transition hover:bg-slate-50/80">
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          <div className="min-w-0">
                            <div className="flex flex-wrap items-center gap-2">
                              <span className="truncate text-sm font-semibold text-slate-900">{brand.brand_name}</span>
                      ...
                      <td className="hidden px-6 py-4 align-middle text-sm text-slate-500 md:table-cell">
                        {formatDate(brand.created_at)}
                      </td>
                      <td className="py-4 pr-6 pl-2 align-middle">
                        <div className="ml-auto flex w-fit justify-end gap-1.5 rounded-lg border border-slate-200 bg-slate-50/60 p-1.5">
                          <button
                            type="button"
                            onClick={() => setEditingBrand(brand)}
                            className="inline-flex items-center justify-center gap-1.5 rounded-md border border-slate-200 bg-white px-2.5 py-1.5 text-slate-600 transition hover:border-indigo-200 hover:bg-indigo-50 hover:text-indigo-700"
                          ...
                          <button
                            ...
                            className="inline-flex items-center justify-center gap-1.5 rounded-md border border-red-200 bg-red-50 px-2.5 py-1.5 text-red-600 transition hover:bg-red-100"
      ...
      <ConfirmationModal
        ...
        title="Delete brand"
        description={`Are you sure you want to delete "${pendingDeleteBrand?.brand_name ?? 'this brand'}"? This action cannot be undone.`}
        confirmText="Delete"
        cancelText="Cancel"
        danger
