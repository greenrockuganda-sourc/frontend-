...
export default function Login({ onLogin }: LoginProps) {
  ...
  const validIdentifier = () => {
    // Accept either an email or a phone number for recovery
    if (!email.trim()) { setError('Enter an email address or phone number.'); return false }
    return true
  }
  ...
  const submit = async (event: FormEvent) => {
    ...
    try {
      ...
      if (view === 'forgot-password') {
        ...
        setNotice('If an account exists for this identifier, we have sent password reset instructions.'); return
      }
