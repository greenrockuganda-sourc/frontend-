...
export function registerAuthSessionCallback(callback: (sessionValid: boolean) => void) {
  authTokenUpdater = (token) => callback(Boolean(token))
}
...
export async function resetPassword(payload: { uid: string; token: string; new_password: string; confirm_password: string }) {
...
function normalizeStatusForBackend(status: string) {
...
export async function updateOrderStatus(token: string, orderId: string, status: string) {
  const canonicalStatus = normalizeStatusForBackend(status)
